...
WebUI.selectOptionByValue(findTestObject('Button/Page_Shortlyst/dropdown current or past'), 'current', false)
...
for (int row = 1; row < 5; row++) {
    ...
    for (int collumn = 1; collumn < 4; collumn++) {
        ...
        if (filterExist.toLowerCase().contains('present') && filterExist.toLowerCase().contains(inputedJob)) {
            for (int expRow = 1; expRow < 6; expRow++) {
                ...
                if (expYear.toLowerCase().contains('present')) {
                    ...
                    throw new com.kms.katalon.core.exception.StepFailedException(((((inputedJob + ' as ') + 'present') + 
                    'job is Not Found on Profile ') + row) + collumn)
                }
            }
        } else {
